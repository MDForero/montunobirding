...
const AdviceBanner = ({ data }) => {
  ...
  return (
    <section  className={(show ? 'fixed top-0 left-0 right-0 bottom-0 h-screen flex justify-center items-center  z-50' : 'hidden') }>
      <div className=' relative  lg:w-4/5 w-full m-auto  grid grid-rows-4 h-full lg:h-4/5 border-4 border-green-700 adBanner'>
        <img loading='lazy' src={data?.imagen} className='w-full h-full absolute object-cover top-0 left-0 right-0 bottom-0 -z-10' />
